...
public class JwtKeyUtils {
    ...
    private static class SingletonHolder {
        private static final JwtKeyUtils INSTANCE = new JwtKeyUtils();
    }

    public synchronized static JwtKeyUtils getInstance(String modulus, String privateExponent, String publicExponent) {
        if (priKey == null && pubKey == null) {
            priKey = RsaKeyUtils.getPrivateKey(modulus, privateExponent);
            pubKey = RsaKeyUtils.getPublicKey(modulus, privateExponent);
        }
    ...
    public synchronized static void reload(String modulus, String privateExponent, String publicExponent) {
        priKey = RsaKeyUtils.getPrivateKey(modulus, privateExponent);
        pubKey = RsaKeyUtils.getPublicKey(modulus, publicExponent);
    }

    public synchronized static JwtKeyUtils getInstance() {
        if (pubKey == null && priKey == null) {
            priKey = RsaKeyUtils.getPrivateKey(RsaKeyUtils.modulus, RsaKeyUtils.privateExponent);
            pubKey = RsaKeyUtils.getPublicKey(RsaKeyUtils.modulus, RsaKeyUtils.publicExponent);
        }
        return SingletonHolder.INSTANCE;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(checkToken(value).isStatus());
    }
}
